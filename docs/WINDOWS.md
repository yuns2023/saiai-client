# Windows 使用指南（V2 Preview 0.9.0）

本页适用于 SAIAI CLI V2 Preview `0.9.0`。公开 V2 客户端只维护 schema 2
...
Gateway 已镜像匹配的 `0.9.0` bundle 时，可以在 PowerShell 中运行：
...
也可以固定到本仓的 `saiai-v0.9.0` Release。脚本、manifest 和二进制必须
来自同一个 tag：
...
CLI `0.9.0` 没有 `saiai update` 命令。更新时先退出通过 SAIAI 启动的
